...
class SolrMarc extends \VuFind\RecordDriver\SolrMarc
{
     ...
     * @access private
     */
    private function _processPicaTit()
    {
        $count = '';
        $f036A = array();
        $f036B = array();
        ...
        // Suchmuster/Ersetzung allgemein
        $suchmuster1 = '/--.+--:/';
        $suchmuster2 = '/--.+--/';
        $ersetzung = '';

        // getJournal
        if (strpos($this->picaTit['002@']['0']['$0']['0'], "o") === 1) {
            if (!empty($this->picaTit['039B']['0']['$9']['0']))
                $this->journal['ppn'] = $this->picaTit['039B']['0']['$9']['0'];

            if (!empty($this->picaTit['039B']['0']['$a']['0']))
                $this->journal['prefix'] = $this->picaTit['039B']['0']['$a']['0'];

            if (!empty($this->picaTit['039B']['0']['$8']['0']))
                $this->journal['name'] = preg_replace($suchmuster2, $ersetzung, preg_replace($suchmuster1, $ersetzung, $this->picaTit['039B']['0']['$8']['0']));

            if (!empty($this->picaTit['039B']['0']['$c']['0']) && !isset($this->journal['name']))
                $this->journal['name'] = $this->picaTit['039B']['0']['$c']['0'];

            if (!empty($this->picaTit['031A']['0']['$d']['0']))
                $this->journal['band'] = $this->picaTit['031A']['0']['$d']['0'];

            if (!empty($this->picaTit['031A']['0']['$j']['0']))
                $this->journal['jahr'] = $this->picaTit['031A']['0']['$j']['0'];

            if (!empty($this->picaTit['031A']['0']['$e']['0']))
                $this->journal['kommentar'] = $this->picaTit['031A']['0']['$e']['0'];

            if (!empty($this->picaTit['031A']['0']['$h']['0']))
                $this->journal['seite'] = $this->picaTit['031A']['0']['$h']['0'];
        }

        // getVolumes
        if (strpos($this->picaTit['002@']['0']['$0']['0'], "c") === 1 || strpos($this->picaTit['002@']['0']['$0']['0'], "d") === 1)
            $this->volumes[$this->picaTit['003@']['0']['$0']['0']] = 'allvolumes';

        // getRetroUrl
        // preliminary solution for detection of series
        if (isset($this->picaTit['009R']) && strlen($this->picaTit['009R']['0']['$u']['0']) > 0 &&
            isset($this->picaTit['002@']) && strpos($this->picaTit['002@']['0']['$0']['0'], "r") === 0
        )
            $this->retrourl = array($this->picaTit['009R']['0']['$u']['0'],
                $this->picaTit['009R']['0']['$3']['0']);

        foreach ($this->picaTit as $field => &$fieldarr) {
            if ($field === '009P' || strpos($field, '036') === 0 || strpos($field, '039') === 0) {
                foreach ($fieldarr as &$subfield) {
                    // getSeries
                    ...
                    if (isset($subfield['$9'])) {
                        ...
                        if ($field === '039T') {
                            foreach ($subfield['$9'] as $index => &$value) {
                                $this->reviewed[$value] = $subfield['$8'][$index];
                            }
                        }
                        ...
                        if ($field === '039U') {
                            foreach ($subfield['$9'] as $index => &$value) {
                                $this->review[$value] = $subfield['$8'][$index];
                            }
                        }
                    ...
                    if (preg_match('/^039[BCSEDX]/', $field)) {
                        if (isset($subfield['$a'])) {
                            foreach ($subfield['$a'] as $index => &$value) {
                                // getJBibContext
                                if ((strpos($this->picaTit['002@']['0']['$0']['0'], "b") === 1 || strpos($this->picaTit['002@']['0']['$0']['0'], "d") === 1) && $field === '039B') {
                                    $this->jbibcontext[] = array('9' => isset($subfield['$9']) ? $subfield['$9'][$index] : false,
                                        '8' => isset($subfield['$8']) ? preg_replace($suchmuster2, $ersetzung, preg_replace($suchmuster1, $ersetzung, $subfield['$8'][$index])) : false,
                                        'a' => $subfield['$a'][$index],
                                ...
                                // getJournalPrePost
                                if ($field === '039E') {
                                    $this->journalprepost[] = array('9' => isset($subfield['$9']) ? $subfield['$9'][$index] : false,
                                        '8' => isset($subfield['$8']) ? preg_replace($suchmuster2, $ersetzung, preg_replace($suchmuster1, $ersetzung, $subfield['$8'][$index])) : false,
                                        'a' => $subfield['$a'][$index],
                                        'c' => isset($subfield['$c']) ? $subfield['$c'][$index] : false);
                                    continue;
                                ...
                                // getOtherEditions
                                if ($field === '039D') {
                                    $this->otherEditions[] = array('9' => isset($subfield['$9']) ? $subfield['$9'][$index] : false,
                                        '8' => isset($subfield['$8']) ? preg_replace($suchmuster2, $ersetzung, preg_replace($suchmuster1, $ersetzung, $subfield['$8'][$index])) : false,
                                        'a' => $subfield['$a'][$index],
                                        'c' => isset($subfield['$c']) ? $subfield['$c'][$index] : false);
                                    continue;
                    ...
                    if ($field === '009P') {
                        if (isset($subfield['$3']) || isset($subfield['$u'])) {
                            ...
                            for ($i = 0; $i < $count; ++$i) {
                                ...
                                $this->allTitleLinks[] = array('title' => $title,
                                    'value' => $display,
                                    'link' => $url);
        ...
        // getSeries
        $this->series = array($f036A, $f036B, $f036C, $f036D, $f036F, $f036G);
    }
     ...
     * @access private
     */
    private function _processPicaLocal()
    {
        foreach ($this->picaLocal as $key => &$arr) {
            // With ILN do exact match, without ILN find matching EPNs
            $lok = $this->picaILN ? 'lok: ' . $this->picaEpn . ' ' . $this->picaILN : $this->picaEpn;
            $found = $this->picaILN ? ($key === $lok) ? true : false : (strpos($key, $lok) > -1) ? true : false;
            ...
            if ($found) {
                ...
                foreach ($this->picaLocal[$key] as $field => &$fieldarr) {
                    if ($field === '147I') {
                        foreach ($fieldarr as &$subfield) {
                ...
                $this->levelonedata = array('lvl1klassifikationen' => $klassarray,
                    'lvl1schlagworte' => $schlagarray, 'lvl1zus' => $sumarray);
     ...
     * @access private
     */
    private function _processPicaExp()
    {
        // Proxy falls eingestellt
        $proxyUtil = new ProxyUrlBuilder();
        $withproxy = ($proxyUtil->hasProxy() && $proxyUtil->isRestricted());
        // Qualified name
        $label = "Fulltext";
        foreach ($this->picaExp as $key => &$arr) {
            // With ILN find exact EPN + ILN combinations, without ILN find matching EPNs
            $lok = $this->picaILN ? $this->picaEpn . ' ' . $this->picaILN . ' ' : $this->picaEpn;
            ...
            if (strpos($key, $lok) > -1) {
                        ...
                        foreach ($fieldarr as &$subfield) {
                            ...
                            if ($field === '204P') {
                                if ($subfield['$S']['0'] !== null && $subfield['$S']['0'] === '0')
                                    $withproxy = false;
                                if ($withproxy) {
                                    $this->urls[$proxyUtil->addProxy('http://dx.doi.org/' . $subfield['$0']['0'])] = isset($label) ? $label : 'Volltext DOI';
                                } else {
                                    $this->urls['http://dx.doi.org/' . $subfield['$0']['0']] = isset($label) ? $label : 'Volltext DOI';
                                }
                                break;
                            }
                            ...
                            if ($field === '204R') {
                                if ($subfield['$S']['0'] !== null && $subfield['$S']['0'] === '0')
                                    $withproxy = false;
                                if ($withproxy) {
                                    $this->urls[$proxyUtil->addProxy('http://hdl.handle.net/' . $subfield['$0']['0'])] = isset($label) ? $label : 'Volltext Handle';
                                } else {
                                    $this->urls['http://hdl.handle.net/' . $subfield['$0']['0']] = isset($label) ? $label : 'Volltext Handle';
                                }
                                break;
                            }
                            ...
                            if ($field === '204U') {
                                if ($subfield['$S']['0'] !== null && $subfield['$S']['0'] === '0')
                                    $withproxy = false;
                                if ($withproxy) {
                                    $this->urls[$proxyUtil->addProxy('http://nbn-resolving.de/urn/resolver.pl?urn=' . $subfield['$0']['0'])] = isset($label) ? $label : 'Volltext URN';
                                } else {
                                    $this->urls['http://nbn-resolving.de/urn/resolver.pl?urn=' . $subfield['$0']['0']] = isset($label) ? $label : 'Volltext URN';
                                }
                                break;
                            }
                            ...
                            if ($field === '209S') {
                                if (isset($subfield['$S']) && $subfield['$S']['0'] !== null && $subfield['$S']['0'] === '0')
                                    $withproxy = false;
                                if ($withproxy && !isset($free)) {
                                    $this->urls[$proxyUtil->addProxy($subfield['$u']['0'])] = isset($label) ? $label : 'Volltext';
                                } else {
                                    $this->urls[$subfield['$u']['0']] = isset($label) ? $label : 'Volltext';
                                }
                                break;
                            }
                            ...
                            // Retro URL
                            if ($field === '209U') {
                                $retro_tmp["url"] = $subfield['$u']['0'];
                                $retro_tmp["title"] = $subfield['$3']['0'];
                                $retrourl = $subfield['$u']['0'];
                                /*
                                 * for ticket 174
                                //Get redirect from resolver
                                $headers = get_headers($subfield['$u']['0'], 1);
                                $retro_tmp["url"] = preg_replace('/vorschau/', 'normal', $headers['Location']);
                                $retrourl = $retro_tmp["url"];
                                print_r($retrourl);
                                */
                                $retrotitel = $subfield['$3']['0'];
                                $retro[] = $retro_tmp;
                                //break;
                            }
                            // Provenienz
                            if ($field === '220D') {
                                $provenance['anmerkung'] = $subfield['$a']['0'] . "<br>";
                            }
                ...
                if (strlen($epn) > 0) {
                    $this->copies[$epn] = array('kommentare' => $komarray,
